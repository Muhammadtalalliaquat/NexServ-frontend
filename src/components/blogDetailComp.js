...
export default function BlogDetailPage({ initialBlogData }) {
  ...
  const params = useParams();
    const blogId = initialBlogData?._id;

  const { successMsg, resError } = useSelector((state) => state.bookmark);

  const { title, content, image, tags, createdAt, updatedAt } = initialBlogData;
